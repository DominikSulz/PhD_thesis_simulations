function [Y,tau] = init_diss_all_dim_diff_rank(r,cc,d)
...
for k=1:d
    ...
    Y = set_leaf(Y,U,k,d,cc,r,d_full);
end
Y{end-1} = 1;
if 1==iscell(Y{1})
    s1 = size(Y{1}{end});
...
function [Z] = set_leaf(Y,S,k,d,cc,r,d_full)
...
if d==2
    ...
    if mod(k,2) == 0 % even k
        if cc==1
            rr = [r(end) r(end) r(end)];
        elseif cc==2
            rr = [r(end) r(end) r(end-1)];
        elseif cc==3
            rr = [r(end) r(end) r(end-1)];
        end
...
elseif d==3 
    ...
    % fit r_vec for recursive structure
    ...
    if k <= dim_left
        k_new = k;
        d_new = dim_left;
        Z{1} = set_leaf(Y{1},S,k_new,d_new,cc,r_new,d_full);
    else
        k_new = k - dim_left;
        d_new = dim_right;
        Z{2} = set_leaf(Y{2},S,k_new,d_new,cc,r_new,d_full);
    end
