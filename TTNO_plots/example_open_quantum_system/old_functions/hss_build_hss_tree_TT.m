function H = hss_build_hss_tree_TT(m, n, block_size, partitionm, partitionn)
...
%     end
    ...
    H.ml = partitionm(1);
    H.nl = partitionn(2:end);
    
    H.mr = H.ml; % H.mr = m - H.ml;
    H.nr = H.nl; % H.nr = n - H.nl;
    
    if (H.ml > 0 && H.nl > 0) && (H.mr > 0 && H.nr > 0)
        H.leafnode = 0;
        H.A11 = build_hss_tree_partition_rec(H.ml, H.nl, partitionm(1), partitionn(1));
        H.A22 = build_hss_tree_partition_rec(H.mr, H.nr, partitionm(2:end) - H.ml, partitionn(2:end) - H.nl);
